...
export default function LobbyPage() {
  ...
  const calcularBalance = async (pacienteId: number): Promise<BalanceResumen> => {
    try {
      ...
      const ingresos = data.filter((e) => e.tipo_movimiento === "ingreso").reduce((sum, e) => sum + e.volumen_ml, 0)
      const egresos = data.filter((e) => e.tipo_movimiento === "egreso").reduce((sum, e) => sum + e.volumen_ml, 0)
  ...
  const cargarPacientes = async () => {
    try {
      ...
      if (data && data.length > 0) {
        const pacientesConIMC = data.map((paciente) => ({
          ...paciente,
          imc: paciente.peso / Math.pow(paciente.talla / 100, 2),
        }))

        // Mostrar pacientes inmediatamente con balance en 0
        const pacientesIniciales = pacientesConIMC.map((paciente) => ({
          ...paciente,
          balance: { total_ingresos_ml: 0, total_egresos_ml: 0, balance_ml: 0 },
          loading: true,
        }))
        ...
        const pacientesConBalance = await Promise.all(
          pacientesConIMC.map(async (paciente) => {
        ...
        {loading ? (
          ...
          <Card>
            <CardContent className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <h3 className="mb-2 text-lg font-semibold">No hay pacientes registrados</h3>
              <p className="mb-4 text-sm text-muted-foreground">Comienza registrando un nuevo paciente</p>
              <NuevoPacienteDialog onPacienteCreado={cargarPacientes} />
            </CardContent>
          </Card>
        ) : (
                ...
                <Link key={paciente.id} href={`/patient/${paciente.id}`}>
                  ...
                  >
                    <CardHeader className="pb-3">
                      <div className="flex items-center justify-between">
                        <CardTitle className="text-xl font-semibold">{paciente.nombre}</CardTitle>
                    ...
                    <CardContent>
                      <div className="space-y-4">
                        <div className="space-y-1 text-sm text-muted-foreground">
                          <p>
                            Edad: <span className="font-medium text-foreground">{paciente.edad} años</span> | Peso:{" "}
                            <span className="font-medium text-foreground">{paciente.peso} kg</span>
                          </p>
                          <p>
                            Talla: <span className="font-medium text-foreground">{paciente.talla} cm</span> | IMC:{" "}
                            <span className="font-medium text-foreground">{paciente.imc?.toFixed(2) || "N/A"}</span>
                          </p>
                        </div>

                        <div className="space-y-2 border-t pt-3">
                          <p className="text-sm font-medium text-muted-foreground">Balance Hídrico Total Acumulado</p>
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <Badge
                              variant={
                                balanceStatus.color === "negativo"
                                  ? "destructive"
                                  : balanceStatus.color === "positivo"
                                    ? "default"
                                    : "secondary"
                              }
                            >
        ...
        <Card className="bg-muted/30">
          ...
          <CardContent className="space-y-2 text-sm text-muted-foreground">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="font-medium text-foreground">{pacientes.length}</span>
            </div>
            <div className="flex items-center justify-between">
              <span>Período de balance mostrado:</span>
              <span className="font-medium text-foreground">Total acumulado</span>
            </div>
          </CardContent>
        </Card>
      </div>
